...
// Importa a lógica do jogo e os Modelos
...
// ==================================================================
// 🔒 TRAVA DE SEGURANÇA (DOMAIN LOCK)
// ==================================================================

// ⚠️ EDITE AQUI: Coloque o domínio do cliente que comprou.
// Se ele tentar rodar em outro lugar, o site bloqueia.
const DOMINIO_AUTORIZADO = "mine-pix.onrender.com"; 

app.use((req, res, next) => {
    const host = req.get('host');
    
    // Permite rodar no Localhost (para seus testes) ou no Domínio Pago
    if (!host.includes(DOMINIO_AUTORIZADO) && !host.includes("localhost")) {
        console.error(`🚫 TENTATIVA DE PIRATARIA BLOQUEADA: ${host}`);
        
        // Retorna erro e trava o script
        return res.status(403).send(`
            <body style="background-color:#0f1923; color:#ff4d4d; font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; flex-direction:column; text-align:center;">
                ...
                <h2>Licença Inválida para o domínio: <span style="color:white">${host}</span></h2>
                <p>Este software é protegido por direitos autorais.</p>
                <p>Adquira sua licença oficial para liberar o acesso.</p>
            </body>
        `);
    }
    next();
...
mongoose.connect(MONGO_URI)
    .then(async () => {
        ...
        if (!settings) {
            await Settings.create({ dailyBonus: 1.00, houseEdge: 0.95 });
...
// ❌ ROTA DE SIMULAÇÃO REMOVIDA PARA SEGURANÇA (MODO PRODUÇÃO) ❌
// Se precisar testar, pague R$ 1,00 no PIX real.
