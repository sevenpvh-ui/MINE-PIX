...
async function loadTransactions() {
    ...
    if(data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:10px;">Nenhuma transação.</td></tr>';
        return;
    }

    data.forEach(t => {
        const date = new Date(t.createdAt).toLocaleDateString('pt-BR');
        let typeHTML = t.type === 'deposit' ? '<span style="color:#00e701">Depósito</span>' : '<span style="color:orange">Saque</span>';
        let statusColor = t.status === 'approved' ? '#00e701' : (t.status === 'pending' ? 'orange' : 'red');
        let statusText = t.status === 'approved' ? 'Pago' : (t.status === 'pending' ? 'Pendente' : 'Cancelado');

        const row = `
            <tr style="border-bottom: 1px solid #2f4553;">
                <td style="padding: 8px;">${typeHTML}</td>
                <td style="font-weight:bold;">R$ ${t.amount.toFixed(2)}</td>
                <td style="color: ${statusColor}">${statusText}</td>
                <td style="color: #b1bad3">${date}</td>
            </tr>`;
        tbody.innerHTML += row;
    });
}
...
async function generatePix() {
    ...
    if(res.ok) {
        ...
        const checkInterval = setInterval(async () => { await updateBalance(); }, 5000);
    } else {
        alert(data.error);
    }
}
...
async function simulateDeposit() {
    ...
    const data = await res.json();
    if(res.ok) {
        alert(`✅ Simulado: R$ ${amount} creditado.`);
        updateBalance();
...
async function playRound(index, cellBtn) {
    ...
    if(data.status === 'safe') {
        cellBtn.innerHTML = '<img src="diamond.png" style="width:70%; drop-shadow: 0 0 5px #00e701;" onerror="this.parentNode.innerText=\'💎\'">';
